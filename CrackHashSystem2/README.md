...
Данная лабораторная работа является второй частью проекта **CrackHash**.

В рамках второй лабораторной работы необходимо модифицировать систему таким образом, чтобы обеспечить гарантированную обработку запроса пользователя при условии, что доступен менеджер.

Основная цель работы — повысить отказоустойчивость системы и обеспечить сохранность данных при сбоях отдельных компонентов.

---

## Цель лабораторной работы

Цель работы — реализовать отказоустойчивую версию распределённой системы CrackHash.

Система должна продолжать корректно работать при сбоях:
...
Во второй лабораторной работе взаимодействие между сервисами переносится с прямого HTTP-вызова на обмен сообщениями через RabbitMQ.

Также добавляется база данных MongoDB для хранения состояния запросов.
